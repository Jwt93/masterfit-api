...
// Single source of truth for turning a phone number into the exact string
// used both to store and to match `formData.clientBasics.phone`. Every
// route that saves or looks up a client by phone must use this - not its
// own ad-hoc trim/format - otherwise the same client typing the same
// digits into different pages (signup vs. returning-member login) can
// produce two strings that look identical but aren't, so an exact-match
// query silently finds nothing and either creates a duplicate document or
// fails to find the returning client at all.
...
// Request logging - the app previously logged nothing for a successful
// request (only console.error on failure), so a request that reached the
// server and succeeded was indistinguishable from one that never arrived at
// all when reading the platform logs. Logging every request on completion
// makes that visible going forward.
...
// Health check - actually pings the database on every call rather than just
// checking whether the `db` variable is set. A connection that succeeded at
// startup but has since dropped (e.g. an Atlas free-tier cluster that paused
// itself, a network blip) would still leave `db` truthy forever, so that
// check alone can report "connected" long after writes have started failing.
...
app.post('/api/clients', async (req, res) => {
  try {
    ...
    console.log(`POST /api/clients: branch=${branch} isSubmission=${!!isSubmission} hasId=${!!id} phone=${formData?.clientBasics?.phone || '(none)'} mongoConnected=${!!db}`);
    ...
    if (id && ObjectId.isValid(id)) {
      ...
      console.log(`POST /api/clients: updated by id -> ${result?._id || '(no match found for that id)'}`);
      return res.json({ data: result });
    }

    // Normalize both the value used to match an existing record AND the
    // value actually persisted, so a document saved through this route is
    // never itself the source of a future mismatch.
    if (formData.clientBasics && formData.clientBasics.phone !== undefined) {
      formData.clientBasics.phone = normalizePhone(formData.clientBasics.phone);
    }
    const phone = normalizePhone(formData.clientBasics?.phone);

    // Do not create or match a record while the phone number is still
    // incomplete (fewer than 7 digits) - this prevents duplicate "junk"
    // records from being created on every keystroke before the client
    // finishes typing their phone number. We still return a success
    // response with a temporary in-memory object (not saved) so the
    // frontend doesn't error out - it just won't persist until the phone
    // number is long enough or a real id exists.
    //
    // Critically, this must NEVER apply to isSubmission:true. That flag
    // means the client explicitly pressed Submit on a fully completed
    // form - if their phone number is short for any reason (a typo, an
    // unusual format, a placeholder value), silently discarding a real,
    // explicit submission and returning 200 as if it saved is exactly the
    // kind of failure that looks identical to success in every log and on
    // the client's own screen. A short phone on a genuine submission
    // should still be saved as-is; validating phone format is the
    // frontend's job before Submit is ever reachable, not this endpoint's.
    const digitsOnly = phone.replace(/\D/g, '');
    if (!isSubmission && digitsOnly.length > 0 && digitsOnly.length < 7) {
      console.log(`POST /api/clients: phone still incomplete (${digitsOnly.length} digits) - not saving, autosave pending`);
      return res.json({
        data: {
          _id: null,
          branch,
    ...
    if (phone) {
      ...
      console.log(`POST /api/clients: upserted by phone -> id=${updated?._id} visitCount=${updated?.visitCount}`);
    ...
    console.log(`POST /api/clients: inserted (no phone) -> id=${result.insertedId}`);
    res.json({ data: saved });
  } catch (error) {
    // error.name/code distinguish a dead/unreachable database (e.g. an
    // Atlas cluster that's paused or IP-blocked - MongoServerSelectionError)
    // from a genuine application bug, which a bare error.message can't.
    console.error(`POST /api/clients failed: ${error.name || 'Error'} (code ${error.code ?? 'n/a'}): ${error.message}`);
